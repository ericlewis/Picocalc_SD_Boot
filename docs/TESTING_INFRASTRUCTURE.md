...
3. **Hardware Testing** - Real device validation with pytest-embedded
4. **Continuous Integration** - Automated pipeline with GitHub Actions
...
### 2. UF2 Validation (`tools/check_uf2_crc32.py`)
...
2. **Run hardware tests** (requires connected Pico):
   ```bash
...
2. **Test on real hardware** - Verify actual device behavior
